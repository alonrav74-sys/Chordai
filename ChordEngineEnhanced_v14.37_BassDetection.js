/**
 * ChordEngineEnhanced v14.37 - BASS DETECTION - FULLY FIXED
 * ✅ Fixed: undefined label errors
 * ✅ Fixed: safety checks for all timeline operations
 * ✅ Added: bass-only detection mode
 */
...
class ChordEngineEnhanced {
  ...
  async detect(audioBuffer, options = {}) {
    const opts = {
      ...
      bassOnlyMode: options.bassOnlyMode || false
    };

    if (opts.bassOnlyMode) {
      return await this.detectBassOnly(audioBuffer, opts);
    }
    ...
    // ✅ CRITICAL FIX: Filter invalid events
    ...
    const stats = {
      ...
      extensions: timeline.filter(e => e.label && /[679]|11|13|sus|dim|aug/.test(e.label)).length,
      modulations,
  ...
  async detectBassOnly(audioBuffer, options = {}) {
    const opts = {
      channelData: options.channelData || null,
      sampleRate: options.sampleRate || null,
    ...
    if (opts.progressCallback) opts.progressCallback({ stage: 'extracting_bass', progress: 0.3 });

    const feats = this.extractFeatures(audioData);

    if (opts.progressCallback) opts.progressCallback({ stage: 'tracking_bass', progress: 0.6 });

    const bassTimeline = this.createBassTimeline(feats, opts);

    if (opts.progressCallback) opts.progressCallback({ stage: 'complete', progress: 1.0 });

    const timings = { total: now() - startTotal };
    ...
    return {
      ...
      stats: {
        ...
        averageDuration: bassTimeline.length > 0 
          ? audioData.duration / bassTimeline.length 
          : 0
      },
      timings
    };
  ...
  applyPostProcessing(timeline, key, feats, bpm, opts) {
    ...
    // ✅ FIX: Final safety filter
    return timeline.filter(ev => ev && ev.label && typeof ev.label === 'string' && ev.label.trim());
  }
  ...
  chordTrackingHMMHybrid(feats, key, bassMultiplier, useFullMode = true) {
    ...
    // ✅ FIX: Safety check for candidates
    if (!candidates || candidates.length === 0) {
      console.warn('No candidates generated, using fallback');
      return [{ t: 0, label: 'C', fi: 0 }];
    }

    // ✅ FIX: Ensure all candidates have valid labels
    candidates.forEach((c, i) => {
      if (!c.label || typeof c.label !== 'string' || !c.label.trim()) {
        console.warn(`Candidate ${i} has invalid label, fixing`);
        c.label = 'C';
      }
    });
    ...
    for (let i = 1; i < M; i++) {
      if (states[i] !== cur) {
        // ✅ FIX: Safety check before accessing label
        const candidate = candidates[cur];
        const label = (candidate && candidate.label && typeof candidate.label === 'string') ? candidate.label : 'C';
        timeline.push({ t: start * secPerHop, label, fi: start });
        cur = states[i];
        start = i;
      }
    }

    // ✅ FIX: Safety check for final chord
    const finalCandidate = candidates[cur];
    const finalLabel = (finalCandidate && finalCandidate.label && typeof finalCandidate.label === 'string') ? finalCandidate.label : 'C';
    timeline.push({ t: start * secPerHop, label: finalLabel, fi: start });
  ...
  finalizeTimeline(timeline, key, bpm, feats) {
    // ✅ FIX: Filter invalid events immediately
  ...
  classifyOrnaments(timeline, bpm, feats) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      if (dur < 0.4 * spb && prev && next && prev.label && next.label && prev.label === next.label && ornamentType === 'structural') {
