/**
 * ChordEngineEnhanced v14.37 - BASS DETECTION ADDED
 * ✅ Added separate bass-only detection mode
 * ✅ Original chord detection remains unchanged
 * ✅ הוסרו כפילויות קוד
 * ✅ הוסרו פונקציות לא בשימוש
...
class ChordEngineEnhanced {
  ...
  // ✅ NEW: Main detect with optional bass-only mode
  async detect(audioBuffer, options = {}) {
    const opts = {
      ...
      bassOnlyMode: options.bassOnlyMode || false  // ✅ NEW OPTION
    };

    // ✅ NEW: If bass-only mode requested, use simplified detection
    if (opts.bassOnlyMode) {
      return await this.detectBassOnly(audioBuffer, opts);
    }

    // Original chord detection code continues here...
  ...
  // ✅ NEW: Create timeline of bass note changes
